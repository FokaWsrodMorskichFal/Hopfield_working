...
if __name__ == '__main__':
    ...
    # 5 - OCRA, 6 - small
    train, dims = read_csv.read_patterns('../data/' + datasets[5])
    #train = train[[True, False, False, False, True]]
    num_of_patterns = train.shape[0]
    
    for i in range(num_of_patterns):
        display.save_img(train[i], dims, "../test/train/p" + str(i+1) + ".png")

    X = []
    for i in range(num_of_patterns):
        X.append(read_csv.noise(train[i], 0.1))
        display.save_img(X[-1], dims, "../test/noise/n" + str(i+1) + ".png")
    ...
    for i in range(num_of_patterns):
        ...
        display.save_img(last_x, dims, "../test/hebb/h" + str(i+1) + ".png")
